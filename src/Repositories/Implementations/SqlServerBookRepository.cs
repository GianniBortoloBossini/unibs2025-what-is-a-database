...
/// <summary>
/// ✅ SOLUZIONE: Implementazione specifica per SQL Server del repository
/// Contiene tutte le query SQL specifiche per SQL Server con naming conventions appropriate
/// Separata dalla logica di business e facilmente sostituibile
/// </summary>
public class SqlServerBookRepository : IBookRepository
{
    private readonly IDbConnectionFactory _connectionFactory;

    public SqlServerBookRepository(IDbConnectionFactory connectionFactory)
    {
    ...
    public async Task<IEnumerable<Book>> SearchAsync(string? title, string? author)
    {
        ...
        {
            conditions.Add("AND Title LIKE @Title");
            parameters.Add("Title", $"%{title}%");
        }

        if (!string.IsNullOrWhiteSpace(author))
        {
            conditions.Add("AND Author LIKE @Author");
